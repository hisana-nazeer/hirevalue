...
export async function POST(request) {
  try {
    // Parse JSON body containing prompt text (resume text)
    const { prompt } = await request.json();

    if (!prompt || prompt.trim().length < 50) {
      return NextResponse.json({ error: "Prompt text too short" }, { status: 400 });
    }
    ...
    const messages = [
      {
        role: "user",
        content: `Analyze this resume and provide a detailed report:\n\n${prompt}`
      }
    ];

    // Call OpenAI chat completion with GPT-4 or GPT-3.5-turbo
    ...
    // Return the analyzed result as JSON
