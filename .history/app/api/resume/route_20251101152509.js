...
export async function POST(request) {
  try {
    ...
    console.log("API /resume received prompt:", prompt?.slice(0, 100));
    const { prompt } = body;
    if (!prompt || prompt.trim().length < 50) {
      return NextResponse.json(
        { error: "Prompt text too short for meaningful analysis." },
        { status: 400 }
      );
    }
    ...
    const messages = [
      {
        role: "user",
        content: `You are an expert career counselor and resume analyst.\nAnalyze this resume text carefully and provide a summary of strengths, weaknesses, and suggestions for improvement:\n\n${prompt}`,
      },
    ];
